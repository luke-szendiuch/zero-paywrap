...
- **charge intent** — single-shot payment. Buyer signs a challenge, mppx settles atomically on-chain at verify time. No channel, no cumulative accounting. Seller wallet must hold a small USDC balance to pay gas (it's the seller that submits the settlement tx — the buyer pays the *price*, not the *gas*). ~$0.10 seeds hundreds of verifies; after first sale the wallet is net positive.
