...
- **Service indexer requires both `paywrap.json` AND `openapi.json`** — Zero's `/v1/register` crawler probes `/openapi.json` to enumerate endpoints. If absent, the probe returns "no mpp/x402 signal at the provided URL" and the service is silently dropped from the index regardless of how good `paywrap.json` is. Always serve both: `buildPaywrapJson` for `/.well-known/paywrap.json` and `buildOpenApiSpec` for `/openapi.json`. Both helpers live in `@zeroclickai/paywrap/manifest` and accept the same manifest shape. Diagnosed live during zero-deepgram launch 2026-04-28 — paywrap.json was correct, openapi.json was missing, registration job 1690 returned 0 capabilities.
