...
- **Service indexer requires `openapi.json`** — Zero's `/v1/register` crawler probes `/openapi.json` to enumerate endpoints and read pricing. If absent, the probe returns "no mpp/x402 signal at the provided URL" and the service is silently dropped from the index. `paywrap.json` is internal/legacy and not part of the public discovery contract — do not rely on it for registration. Use `buildOpenApiSpec` from `@zeroclickai/paywrap/manifest` to emit `/openapi.json`. Diagnosed live during zero-deepgram launch 2026-04-28 — openapi.json was missing, registration job 1690 returned 0 capabilities.
